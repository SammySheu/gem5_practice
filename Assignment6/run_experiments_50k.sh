#!/bin/bash
#
# Run DAXPY TLP experiments with fully parallel scalable binary (50K elements).
#
# Uses daxpy_mt_scalable.c which parallelizes init + DAXPY + validate.
# Sweeps FloatSimdFU (opLat + issueLat = 7) across multiple thread counts.
#
# Usage: bash configs/practice/Assignment6/run_experiments_50k.sh
...
BINARY="${SCRIPT_DIR}/daxpy_mt_scalable_arm"
RESULTS_DIR="${SCRIPT_DIR}/results_50k"
CSV_FILE="${SCRIPT_DIR}/results_50k.csv"
ARRAY_SIZE=50000
...
# -----------------------------------------------------------------------
# Step 1: Compile the scalable DAXPY binary if needed
# -----------------------------------------------------------------------
if [ ! -f "${BINARY}" ]; then
    echo "Compiling scalable DAXPY binary..."
    aarch64-linux-gnu-gcc -static -O2 -pthread \
        -o "${BINARY}" "${SCRIPT_DIR}/daxpy_mt_scalable.c"
